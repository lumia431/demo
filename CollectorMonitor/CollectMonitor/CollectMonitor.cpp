...
int CollectMonitor::Init()
{
	...
	ReadTag();
	LOGINFO("tag.size = {} ", m_tagData.size());
	try
	{
		m_conn = PQsetdbLogin(m_pgIp.c_str(), m_pgPort.c_str(), NULL, NULL, m_pgDbName.c_str(), m_pgUser.c_str(), m_pgPwd.c_str());
		if (m_conn == NULL)
		{
			LOGINFO(" connect pgsql error ,ip ={} , port = {} , dbName = {},user = {},pwd = {}",
				m_pgIp.c_str(), m_pgPort.c_str(), m_pgDbName.c_str(), m_pgUser.c_str(), m_pgPwd.c_str());
			std::cout << "connect pgsql error" << std::endl;
			return -1;
		}
		PGresult * result = PQexec(m_conn, "select * from tdmacquista");//执行SQL语句，关键字要大写
		for (int i = 0; i < PQntuples(result); i++)//结果行数
		{
			m_taskList.emplace_back(PQgetvalue(result, i, 0));
		}
		LOGDEBUG("task.size = {} ", m_taskList.size());
		...
		if (m_dbApiJt.Connect(m_dbInfoJt) < 0)
		{
			LOGINFO("connect jt rtdb error");
			std::cout << "connect rtdb error" << std::endl;
			return -1;
		}
		httpClientPtr = std::make_shared<SimpleWeb::Client<SimpleWeb::HTTP>>(m_urlAddr.c_str());
		std::thread thrCheck(&CollectMonitor::CheckTimeThr, this);
		std::thread thrSend(&CollectMonitor::SendThr, this);
		thrCheck.detach();
		thrSend.detach();
	}
...
int CollectMonitor::Run()
{
	...
	const std::string CLIENT_ID = "monitor" + std::to_string(rand());
	const auto TIMEOUT = std::chrono::seconds(60);
	m_topic = "sys/task/status";
	...
	try
	{
		...
		while (true)
		{
			...
			auto msg = cli.try_consume_message_for(TIMEOUT);
			if (!msg)
			{
				continue;
			}
			std::string topicStr = msg->get_topic();
			buffer = msg->to_string();
			//LOGDEBUG("recv buffer = {}", buffer);
			if (UnpackBuffer(buffer) < 0)
			{
				std::cout << "send buffer fail!" << std::endl;
			}
			//Sleep(1000);
		}
		cli.unsubscribe(m_topic)->wait();
		cli.stop_consuming();
		cli.disconnect()->wait();
	}
	catch (const mqtt::exception& exc)
	{
		LOGINFO("mqtt exception = {}", exc.what());
...
int CollectMonitor::ReadTag()
{
	...
	{
		std::vector<std::string> tmpList1;
		m_tagData[lineStr] = 0;
		m_rtdbTimeOut[lineStr] = 0;
		SplitStr(lineStr, tmpList1, ",");
		std::vector<std::string> tmpList2;
		SplitStr(tmpList1[0], tmpList2, ":");
		if (tmpList2[1] == "CollectTask")
		{
			data.taskType = 0;
		}
		if (tmpList2[1] == "TransmitTask")
		{
			data.taskType = 2;
		}
		if (tmpList2[1] == "VibrationTask")
		{
			data.taskType = 3;
		}
		m_bufferMap[tmpList2[2]] = data;
		m_alarmCounts[tmpList2[2]] = 100;
		if (tmpList1.size() > 1)
		{
			m_queryTags[tmpList2[2]] = StringToWstring(tmpList1[1]);
		}
	}
