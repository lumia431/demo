	...
	: CDialogEx(IDD_HISVALUESET_DIALOG, pParent)
{
	...
	my_logger_ = spdlog::basic_logger_mt<spdlog::async_factory >("hisvalue", "./log.txt");
	//当遇到错误级别以上的立刻刷新到日志
...
void CHisValueSetDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_STATIC_TAGID, m_tagName);
...
void CHisValueSetDlg::OnBnClickedStart()
{
	...
	info.my_logger_ = my_logger_;
	info.readMax = m_readMax;
...
void CHisValueSetDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (m_setHis->disConnectFlg)
	{
		MessageBox("trendDB connect fail!");
	}
	...
	m_percent = buf;
	CString tmp(m_setHis->tagName_.c_str());
	m_tagName = tmp;
	UpdateData(false);
...
void CHisValueSetDlg::ReadCfg()
{
	...
	if (!file)
	{
		...
		WritePrivateProfileString("Config", "ReadMax", "100000", "./config.ini");
	}
	...
	m_readMax = GetPrivateProfileInt("Config", "ReadMax", 0, lpPath);
	delete[] lpPath;
	m_addr = (CString)addr;
	m_usr = (CString)user;
	m_pwd = (CString)pwd;
...
void CHisValueSetDlg::WriteCfg()
{
	...
	WritePrivateProfileString("Config", "ReadMax", std::to_string(m_readMax).c_str(), "./config.ini");
}
